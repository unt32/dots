...
# default pager, used both in the shell mode and in the one-shot mode
...
if [ "$shell_mode" != yes ]; then
  ...
  curl -s "${CHTSH_URL}"/"$(get_query_options "$query")" > "$TMP1"

  lines=$(tput lines 2>/dev/null)

  # only page when stdout is an actual terminal (not a pipe/redirect)
  # and the response is longer than the visible screen
  if [ -t 1 ] && [ -n "$lines" ] && [ "$(wc -l < "$TMP1")" -ge "$lines" ]; then
    ${PAGER:-$defpager} "$TMP1"
  else
    cat "$TMP1"
  fi
